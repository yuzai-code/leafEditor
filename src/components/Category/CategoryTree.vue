<template>
  <div class="category-tree">
    ...
    <CategoryContextMenu
      ...
      @delete-note="(node) => emitContextMenuAction('delete-note', node)"
...
watch(
  [isAddingChildTo, isAddingNoteTo],
  async ([newCategoryValue, newNoteValue], [oldCategoryValue, oldNoteValue]) => {
    // 处理新增分类的情况
    const newlyAddedCategoryIds = Object.keys(newCategoryValue).filter(
      (id) => newCategoryValue[id] && (!oldCategoryValue[id] || !oldCategoryValue[id])
    );
    ...
    // 处理新增笔记的情况
    const newlyAddedNoteIds = Object.keys(newNoteValue).filter(
      (id) => newNoteValue[id] && (!oldNoteValue[id] || !oldNoteValue[id])
    );
...
const startAddChild = (node: ContextMenuNode) => {
  ...
  const currentAddingCategoryIds = Object.keys(isAddingChildTo).filter(
    (id) => isAddingChildTo[id]
  );
...
const generateDefaultCategoryName = (parentId?: string): string => {
  ...
  if (parentId) {
    ...
    if (childrenContainer) {
      // 获取子节点区域中的分类名称
      const childCategories = childrenContainer.querySelectorAll(
        ".category-item .text-gray-700.text-sm"
      );
    ...
    // 获取根级分类名称
    const rootCategories = document.querySelectorAll(
      ".category-tree > .category-list > div > .category-item .text-gray-700.text-sm"
    );
...
const startAddNote = (node: ContextMenuNode) => {
  ...
  // 如果已经有其他分类正在添加笔记，取消它
  const currentAddingNoteIds = Object.keys(isAddingNoteTo).filter(
    (id) => isAddingNoteTo[id]
  );
...
const generateDefaultNoteName = (parentId: string): string => {
  ...
  if (parentId) {
    ...
    if (childrenContainer) {
      ...
      const childNotes = childrenContainer.querySelectorAll(
        ".category-item .text-gray-700.text-sm"
      );
...
const focusNewNoteInput = async (parentId: string) => {
  ...
  const tryFocus = (attemptCount = 1, maxAttempts = 5) => {
    ...
    const inputByContainer = document.querySelector(
      containerSelector
    ) as HTMLInputElement;
    if (inputByContainer) {
